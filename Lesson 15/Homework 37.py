class Rectangle:

    def __init__(self, width, height):
        self.width = width
        self.height = height

    def get_square(self):
        return self.width * self.height

    def __eq__(self, other):
        return self.get_square() == other.get_square()

    def __add__(self, other):
        # Находим новые стороны прямоугольника через площадь и другую сторону
        s = self.get_square() + other.get_square()
        side_a = s / (self.width + other.width)
        side_b = round(s / side_a)
        return Rectangle(side_a, side_b)

    def __mul__(self, n):
        return Rectangle(self.width * n / 2, self.height * n / 2)

    def __str__(self):
        return f'{self.get_square()}'


r1 = Rectangle(2, 4)
r2 = Rectangle(3, 6)
assert r1.get_square() == 8
assert r2.get_square() == 18

r3 = r1 + r2
assert r3.get_square() == 26
#
r4 = r1 * 4
assert r4.get_square() == 32

# Мой проверочный пример
r5 = Rectangle(5, 7)
r6 = Rectangle(2, 8)
assert r5.get_square() == 35
assert r6.get_square() == 16

r7 = r5 + r6
assert r7.get_square() == 51
